...
#### 구간 합
![구간 합 세그먼트 트리](../assets/segment_tree_sum.svg)
...
![Lazy 세그먼트 트리](../assets/lazyseg.svg)
...
![머지 소트 트리](../assets/merge_sort.svg)
...
Node merge(const Node& l, const Node& r) {
    int delta = min(l.open, r.close);
    return {
        l.matched + r.matched + delta,
        (l.open - delta) + r.open,
        l.close + (r.close - delta)
    };
}
```
...
* **Dynamic DP 확장: (min, +) 트로피컬 행렬 (Tropical Semiring)**:
  * 덧셈을 $\min$, 곱셈을 $+$로 대응시키는 Tropical Semiring에서도 결합법칙이 완벽히 성립함:
    $$(A \otimes B)_{i, j} = \min_{k} (A_{i, k} + B_{k, j})$$
  * **대표 문제**: Codeforces 750E (2016 부분수열, $5 \times 5$ 행렬), AtCoder ABC 246 Ex (01? Queries, $3 \times 3$ 행렬).
...
![동적 세그먼트 트리](../assets/dynamic_seg.svg)
...
![Persistent Segment Tree](../assets/pst.svg)

* Dynamic Segment Tree의 응용. 값이 갱신될 때 기존 노드를 덮어쓰지 않고 새로운 노드를 생성하여(경로 복제, Path Copying) 과거의 트리 상태를 모두 보존한다.
...
| 구조 | 주 해결 상황 | 시간 복잡도 | 공간 복잡도 | 핵심 특징 |
| :--- | :--- | :---: | :---: | :--- |
| **Dynamic Seg** | $X \le 10^9$ 대규모 좌표 | $O(\log X)$ | $O(Q \log X)$ | 좌표 압축 불필요, 온라인 처리 |
| **PST (영속 세그)** | 과거 시점 질의, 구간 $K$번째 수 | $O(\log N)$ | $O(N \log N)$ | 경로 복제, 2D $\to$ 1D 차원 축소 |
| **2D 세그먼트 트리** | 실시간 2D 점 갱신 & 영역 질의 | $O(\log^2 N)$ | $O(N \log^2 N)$ | 외부 $x$-세그 + 내부 $y$-세그 |

### 실전 선택 가이드
1. 좌표 범위가 방대하지만 갱신이 적다 $\to$ **Dynamic Seg**
2. 과거 이력이 필요하거나 구간 도수가 필요하다 $\to$ **PST**
3. 2차원 점 갱신이 동적으로 발생한다 $\to$ **2D 세그**
* **알고리즘 최적화 팁**: 질의가 오프라인이라면 2D 세그 대신 **스위핑 + 1D 세그**를, 시간순 쿼리라면 **PST**를 우선 고려하는 것이 구현 난이도와 상수를 크게 낮춥니다.

---

## 9. 추천 실전 문제

- [CF 242E — XOR on Segment](https://codeforces.com/problemset/problem/242/E) (비트별 세그트리)
- [CF 380C — Sereja and Brackets](https://codeforces.com/problemset/problem/380/C) (괄호 매칭 모노이드)
- [JG 2798 — 금광 세그](https://jungol.co.kr/problem/2798) (최대 연속 구간 합)
- [LC Point — Point Set Range Composite](https://judge.yosupo.jp/problem/point_set_range_composite) (일차함수 합성)
- [CF 750E — New Year and Old Subsequence](https://codeforces.com/problemset/problem/750/E) (트로피컬 행렬 DP)
- [ABC 246Ex — 01? Queries](https://atcoder.jp/contests/abc246/tasks/abc246_h) (상태 전이 행렬 Dynamic DP)
- [CF 915E — Physical Education Lessons](https://codeforces.com/problemset/problem/915/E) ($N=10^9$ 동적 세그)
- [ABC 309F — Box in Box](https://atcoder.jp/contests/abc309/tasks/abc309_f) (3차원 점 지배 판별)
- [CF 1149C — Tree Generator™](https://codeforces.com/problemset/problem/1149/C) (괄호 수열 기반 트리 지름)
- [JG 3939 — High Card Low Card](https://jungol.co.kr/problem/3939) (세그트리 그리디 매칭)

---

## 10. 추가 심화: 펜윅 트리 (Fenwick Tree / BIT)

### BIT (Fenwick Tree)
* 세그먼트 트리보다 구현이 매우 짧고 상수가 작으며 메모리를 절반만 쓰는 자료구조.
* 단점: 역원이 존재하는 아벨군(Abelian group)에서만 Prefix sum을 이용해 구간 쿼리($Sum[R] - Sum[L-1]$)가 가능하다. (즉, 구간 최댓값 쿼리 등에는 사용이 매우 까다로움)

### Range Update, Range Query BIT
* Lazy Propagation 없이도 두 개의 BIT를 이용해 $O(\log N)$에 Range Update와 Range Query를 모두 지원하는 수학적 트릭.
* 식 $\sum_{i=1}^{X} A[i]$를 전개하여 상수항 비트(BIT1)와 일차항 비트(BIT2)로 나누어 관리하는 기법.
  $$S(X) = \sum_{i=1}^{X} A[i] = (X + 1) \sum_{i=1}^{X} D[i] - \sum_{i=1}^{X} i \cdot D[i]$$
* **갱신 및 질의 계산 예시**:
  * 구간 $[L, R]$에 $V$를 더할 때:
    * $\text{BIT}_1$ (차 배열 $D$ 관리): $L$에 $+V$, $R+1$에 $-V$ 갱신.
    * $\text{BIT}_2$ ($i \cdot D[i]$ 관리): $L$에 $+L \cdot V$, $R+1$에 $-(R+1) \cdot V$ 갱신.
  * 질의 시:
    * $\text{prefix\_sum}(A, X) = (X + 1) \cdot \text{query}(\text{BIT}_1, X) - \text{query}(\text{BIT}_2, X)$
    * 구간 합 $[L, R]$은 $\text{prefix\_sum}(A, R) - \text{prefix\_sum}(A, L-1)$로 $O(\log N)$에 도출.

### Segment Tree Beats
* 기존 세그트리나 Lazy 세그로 처리하기 어려운 구간 조건부 갱신(예: $A_i \leftarrow \min(A_i, X)$)을 amortization 분석을 바탕으로 처리하는 기법.